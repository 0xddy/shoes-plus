...
impl Assembler {
    ...
    fn defragment(&mut self) {
        // Coalesce small contiguous chunks even when they already use their allocation well.
        // Otherwise a reader that falls behind can hit MAX_CHUNKS without any actual gaps.
        let min_chunk_size = self
            .buffered
            .div_ceil(MAX_CHUNKS)
            .max(MIN_RETAINED_CHUNK_SIZE);
...
/// Minimum size of a defragmented chunk retained without coalescing.
///
/// Keep already large chunks in place to avoid repeatedly copying the same data.
...
mod test {
    ...
    fn ordered_lossless_stream_is_not_rejected() {
        ...
        for index in 0..(2 * COMPACT_THRESHOLD) {
            x.insert((index * FRAME_LEN) as u64, data.clone(), FRAME_LEN)
        ...
        assert_eq!(read, 2 * COMPACT_THRESHOLD * FRAME_LEN);
    }

    #[test]
    fn mtu_sized_upload_survives_reader_backpressure() {
        assert_mtu_backlog_readback(false);
    }

    #[test]
    fn mtu_sized_upload_survives_one_missing_prefix_byte() {
        assert_mtu_backlog_readback(true);
    }

    fn assert_mtu_backlog_readback(withhold_prefix: bool) {
        // Almost 7 MiB fits the configured 8 MiB HY2 stream window. Each ordinary
        // full-sized frame owns its allocation, as happens with UDP packets.
        const FRAME_LEN: usize = 1170;
        const PACKET_LEN: usize = 1200;
        const FRAME_COUNT: usize = 3 * COMPACT_THRESHOLD;
        let mut x = Assembler::new();
        let prefix_len = usize::from(withhold_prefix);
        let mut expected = Vec::with_capacity(prefix_len + FRAME_LEN * FRAME_COUNT);
        if withhold_prefix {
            expected.push(0xab);
        }
        for index in 0..FRAME_COUNT {
            let frame = Bytes::from(vec![(index % 251) as u8; FRAME_LEN]);
            expected.extend_from_slice(&frame);
            x.insert((prefix_len + index * FRAME_LEN) as u64, frame, PACKET_LEN)
                .expect("normal receive-window backlog must not count as excessive gaps");
            assert!(x.data.len() <= COMPACT_THRESHOLD);
            if withhold_prefix {
                assert!(x.read(usize::MAX, true).is_none());
            }
        }
        if withhold_prefix {
            x.insert(0, Bytes::from_static(&[0xab]), PACKET_LEN)
                .unwrap();
        }
        let mut actual = Vec::with_capacity(expected.len());
        // Also exercise partial reads across the coalesced chunk boundaries.
        while let Some(chunk) = x.read(4093, true) {
            assert_eq!(chunk.offset, actual.len() as u64);
            actual.extend_from_slice(&chunk.bytes);
        }
        assert_eq!(actual, expected);
        assert_eq!(x.bytes_read(), expected.len() as u64);
    ...
    #[test]
    fn genuinely_disjoint_spans_still_obey_existing_limit() {
        for spans in [MAX_CHUNKS, MAX_CHUNKS + 1] {
            let mut x = Assembler::new();
            for index in 0..spans {
                x.insert(1 + index as u64 * 4, Bytes::from_static(b"abc"), 3)
                    .unwrap();
            }
            // Duplicates trigger compaction without adding new disjoint spans.
            // The 1024/2048 resource bounds are deliberately unchanged.
            let mut rejected = false;
            for _ in 0..COMPACT_THRESHOLD {
                if x.insert(1, Bytes::from_static(b"abc"), 3).is_err() {
                    rejected = true;
                    break;
                }
                assert!(x.data.len() <= COMPACT_THRESHOLD);
            }
            assert_eq!(rejected, spans > MAX_CHUNKS);
        }
    }
