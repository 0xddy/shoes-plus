...
use crate::{
    ...
    connection_events::{self, EventSender},
    incoming::Incoming,
...
struct ConnectionSet {
    ...
    senders: FxHashMap<ConnectionHandle, EventSender<ConnectionEvent>>,
...
impl ConnectionSet {
    fn insert(
        ...
        let (send, recv) = connection_events::channel();
                        ...
                        while !data.is_empty() {
                            ...
                            match endpoint.handle(
                                ...
                                Some(DatagramEvent::ConnectionEvent(handle, event)) => {
                                    // An overloaded connection drops only its own network
                                    // packets; QUIC recovers reliable data, but not DATAGRAMs.
                                    // Local control events use `send` and bypass this budget.
                                    ...
                                    let _ = self
                                        ...
                                        .get_mut(&handle)
                                        .unwrap()
                                        .send_packet(ConnectionEvent::Proto(event));
                                }
